...
- [x] 将现有导航歌词页改造成真正的沉浸式全屏。
- [x] 支持点击首页歌词卡片展开、手势关闭。
- [x] iPhone 竖屏突出当前歌词和上下文。
- [x] iPad 10 原生自适应布局，支持横屏。
- [x] 加入歌词字号、行距、翻译显示和偏移调节。
- [x] 自动滚动时避免用户手动浏览被强制拉回。
- [x] 用户停止拖动 4 秒后恢复跟随，也可手动立即恢复。
- [x] 正确处理超长中文句子、双语行和无时间轴歌词。
...
- [x] 灵动岛紧凑态无论逐字开关状态都按行执行一次行首到行尾动画；动画按该行剩余时间自动压缩，并为行尾保留完整字符余量。可容纳的短歌词（约 12 个英文字符或 6 个汉字）省略行尾状态提交，并按文字实际宽度收缩右侧区域，避免短歌词仍撑宽整个岛。展开态取消歌词行数限制并允许自动换行。
