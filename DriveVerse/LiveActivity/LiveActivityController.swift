...
final class LiveActivityController {
    ...
    func sync(state: NowPlayingState?, position: LyricsPosition?, hasSyncedLyrics: Bool) {
        ...
        if startsLineMarquee {
            lineMarqueeDuration = Self.marqueeDuration(
    ...
    /// Fit the single-update insertion transition before the next lyric
    /// replaces it. Long lines retain the tested 1.8 s cap; short lines
    /// proportionally compress the pass.
    private static func marqueeDuration(
        remainingMs: Int?
    ) -> TimeInterval {
        ...
        return min(1.8, max(0.12, available))
    }
